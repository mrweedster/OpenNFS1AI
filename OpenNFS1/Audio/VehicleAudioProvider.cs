...
namespace OpenNFS1
{
    class VehicleAudioProvider
    {
        ...
        private SoundEffect LoadWav(string path)
        {
            using (var stream = System.IO.File.OpenRead(path + ".wav"))
            {
        ...
        public void Initialize()
        {
            ...
            SoundEffect temp;
			temp = LoadWav(String.Format(BasePath + "/Vehicles/{0}/engine-on-low", _car.Descriptor.Name));
            _engineOnLow = temp.CreateInstance();
			//.Play(0.3f, 0, 0);
			temp = LoadWav(String.Format(BasePath + "/Vehicles/{0}/engine-on-high", _car.Descriptor.Name));
			_engineOnHigh = temp.CreateInstance(); // temp.Play(0.3f, 0, 0);
			temp = LoadWav(String.Format(BasePath + "/Vehicles/{0}/engine-off-low", _car.Descriptor.Name));
			_engineOffLow = temp.CreateInstance(); //temp.Play(0.3f, 0, 0);
			temp = LoadWav(String.Format(BasePath + "/Vehicles/{0}/engine-off-high", _car.Descriptor.Name));
			_engineOffHigh = temp.CreateInstance(); //temp.Play(0.3f, 0, 0);

			temp = LoadWav(BasePath + "/Vehicles/common/grass_slide");
			_offRoadInstance = temp.CreateInstance(); //temp.Play(0.3f, 0, 0, true);
            _offRoadInstance.Pause();

			_skids.Add(LoadWav(BasePath + "/Vehicles/common/skid1"));
			_skids.Add(LoadWav(BasePath + "/Vehicles/common/skid2"));
			_skids.Add(LoadWav(BasePath + "/Vehicles/common/skid3"));
			...
			_gearChange = LoadWav(String.Format(BasePath + "/Vehicles/{0}/gear-change", _car.Descriptor.Name));
