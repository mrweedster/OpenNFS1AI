...
namespace OpenNFS1.Vehicles
{
	class Vehicle
	{
		...
		public Vehicle(string cfmFile)
		{
			...
			// Discard pixels with alpha=0 (the NFS1 palette transparency key 0xFF is
			// decoded to alpha=0 in BitmapLoader). Without this the default AlphaFunction
			// is Always — every pixel draws including transparent arch cutouts, which
			// appear black over the wheels when they turn into the wheel arch.
			_effect.AlphaFunction = Microsoft.Xna.Framework.Graphics.CompareFunction.Greater;
		...
		private void UpdateSteering()
		{
			...
			// ── TRUE ANALOG STEERING ────────────────────────────────────────────
			// The target wheel angle is directly proportional to the analog input:
			//   50 % stick → 50 % of MaxSteeringLock  (position-based, not rate-based)
			//
			// A slew-rate limiter (SteeringSpeed) prevents instant snapping so the
			// physics stay stable, while still being fast enough to feel responsive.
			// Centering (no input) uses the same slew rate for symmetry.

			float targetWheel = SteeringInput * MaxSteeringLock;
			float maxDelta    = SteeringSpeed * elapsedSeconds;
			float delta       = targetWheel - _steeringWheel;

			if (Math.Abs(delta) <= maxDelta)
				_steeringWheel = targetWheel;            // already at (or past) target
			else
				_steeringWheel += Math.Sign(delta) * maxDelta;
