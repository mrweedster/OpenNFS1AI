	...
	class Race
	{
		...
		public void Update()
		{
			...
			if (PlayerStats.CurrentLap > _nbrLaps)
			{
				...
				// Finish 3 seconds after crossing the line instead of waiting
				// for the car to reach Speed==0 (which could take 30+ seconds).
				if (!_finishTime.HasValue)
					_finishTime = DateTime.Now;
				if ((DateTime.Now - _finishTime.Value).TotalSeconds >= 3.0)
					Finished = true;
			}
