...
namespace OpenNFS1.Parsers
{
	...
	class BitmapEntry
	{
		...
		// Misc[2]/Misc[3] are x/y screen coordinates baked into the original
		// 640x480 game assets.  Scale them to the current resolution so every
		// sprite that uses GetDisplayAt() lands in the right place.
		public Vector2 GetDisplayAt()
		{
			return new Vector2(Misc[2] * GameConfig.ScaleX, Misc[3] * GameConfig.ScaleY);
		}
	}
